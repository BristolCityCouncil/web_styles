...
(function($){
  ...
  $('.js--overlay').overlays();
  $('.js--overlay-dialog').overlays('dialog');
  $('.js--overlay-external-image').overlays('externalImage');
	...
	var breakpoints = Y.uk.gov.bristol.module.extend({
		...
		render: function(){
			...
			var stickyHeight = function() {
	    	    ...
	    	    if ( $( this ).parents(".js-sticky").hasClass( "is-expanded" ) ) {
	    	    	
	    	    	console.log("is-expanded");
	    	    	//$( this ).css("background-color", "green");
	    	    	$( this ).parents(".js-sticky").css("height", "100%");
	    	    	$( this ).parents(".js-sticky").css("overflow", "scroll");
	    	    	
	    	    } else {
	    	    	
	    	    	console.log("is-collapsed");
	    	    	//$( this ).css("background-color", "lime");
	    	    	$( this ).parents(".js-sticky").css("height", "auto");
	    	    	$( this ).parents(".js-sticky").css("overflow", "auto");
	    	...
	    	*/
			...
			this.on('dekstop', function(){
		    	...
		    	//$( "body" ).off("click", ".js-sticky .js-collapse__toggle", stickyHeight);
			...
			this.on('tablet', function(){
				...
				// Sticky Nav
		    	//$( "body" ).on("click", ".js-sticky .js-collapse__toggle", stickyHeight);
    ...
    Y.namespace('uk.gov.bristol').AddressLookup = Y.uk.gov.bristol.module.extend({
    	...
    	options: {
			...
			dynamicAddressTemplate: function(klass, message){ 
				return '<div class="panel panel--affirm panel--date '+klass+'"><span class="panel__title">Your selected address:</span><strong class="panel__copy">'+message+'</strong></div>'; 
			},
    	},
    ...
    Y.namespace('uk.gov.bristol').ParkingPermits = Y.uk.gov.bristol.module.extend({
        ...
        recaluateCost: function() {
            ...
            radiodisabled.each(function() {
                ...
                $(this).closest(".permit").find(".permit__end").html("to " + renewalDate);

            });
            
            // No Choice (Not a radio)
            var isOnly = $("p[data-permit-cost]");
            if(isOnly){
            	var isOnlyRenewalDate = isOnly.find('.permit__cost').attr('data-permit-date');
            	console.log('isOnlyRenewalDate: ', isOnlyRenewalDate);
    			isOnly.closest(".permit").find(".permit__end").html("to " + isOnlyRenewalDate );
            }
            
            if (!total && !checkthisone.length) total = 'unknown';
...
var fixcache = function(){ $('.js--prevent-double').prop("disabled", false).removeAttr("disabled").removeClass('disabled'); };
window.pageshow = function(){ fixcache(); };
...
//console.log("Testing: Resize event (Sticky Nav)");
